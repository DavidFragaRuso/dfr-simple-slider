...
function slide_content_meta_box_callback( $post ) {
    ...
    if( empty( $repeater_slider_content ) ) {
        ...
        $repeater_slider_content['slide'] = array(
            //'num' => '1',
            'img' => '',
            'm-img' => '',
            'bg-color' => '',
    ...
    <div id="repeatable-fieldset-one" class="wrap">
        ...
        foreach( $repeater_slider_content as $field ) {
            ...
            <div class="slide-row ui-state-default">
                <!-- Slider number -->
                <div class="slide-col">
                    <input type="text" class="slide-number" id="slide-num" disabled="disabled" value="# <?php echo $i; ?>">
                    <!--<input type="hidden" name="slide['num'][]" value="<?php //echo absint( $field['slide']['num'] ); ?>">-->
                </div>
                <!-- Slide Background and images -->
                <div class="slide-col">
                    <label><h4><?php _e( 'Slide Backgorund Color', 'dfr_simple_slider' ); ?></h4></label>
                    <input name="slide[bg-color][]" type="text" value="<?php echo esc_attr( $field['slide']['bg-color'] ); ?>" class="color-field" />
                    <h4><?php _e( 'Image upload for desktop', 'dfr_simple_slider' ); ?></h4>
                    <?php 
                    if ( $field['slide']['img'] ) :
                        $slide_img_url = wp_get_attachment_image_url( $field['slide']['img'], 'thumbnail' )
                        ?>
                        ...
                        <input type="hidden" name="slide[img][]" value="<?php echo absint( $field['slide']['img'] ) ?>">
                        <?php
                        else :
                        ?>
                        ...
                        <input type="hidden" name="slide[img][]" value="">
                        <?php
                    endif;
                    ?>
                    <h4><?php _e( 'Image upload for mobile', 'dfr_simple_slider' ); ?></h4>
                    <?php 
                    if ( $field['slide']['m-img'] ) :
                        ...
                        <input type="hidden" name="slide[m-img][]" value="<?php echo absint( $field['slide']['m-img'] ) ?>">
                        <?php
                        else :
                        ?>
                        ...
                        <input type="hidden" name="slide[m-img][]" value="">
                        <?php
                    endif;
                    ?>
                </div>
                ...
                <div class="slide-col">
                    ...
                    <input type="radio" id="align-left" name="slide[align][]" value="left" <?php checked(  $field['slide']['align'], 'left' ); ?> ></input>
                    <label for="align-left"><?php _e( 'Left', 'dfr_simple_slider' ) ?></label>
                    <input type="radio" id="align-center" name="slide[align][]" value="center" <?php checked( $field['slide']['align'], 'center' ); ?> ></input>
                    ...
                    <input type="radio" id="align-right" name="slide[align][]" value="right" <?php checked( $field['slide']['align'], 'right' ); ?> ></input>
                ...
                <div class="slide-col">
                    ...
                    <input type="text" name="slide[title][]" value="<?php  echo esc_attr( $field['slide']['title'] ); ?>" />
                    <label><h4><?php _e( 'Slide Text', 'dfr_simple_slider' ) ?></h4></label>
                    <textarea name="slide[text][]" cols="50" rows="10" maxlength="150" /><?php echo esc_attr( $field['slide']['text'] ); ?></textarea>    
                </div>
                <!-- Slide button and link -->
                <div class="slide-col">
                    <label><h4><?php _e( 'Slide Link', 'dfr_simple_slider' ) ?></h4></label>
                    <input type="url" name="slide[link][]" value="<?php echo esc_attr( $field['slide']['link'] ); ?>" />
                    <label><h4><?php _e( 'Slide button text', 'dfr_simple_slider' ); ?></h4></label>
                    <input type="text" name="slide[btn-text][]" value="<?php echo esc_attr( $field['slide']['btn-text'] ); ?>" />    
                </div>
                <div class="slide-col">
        ...
        <div class="slide-row empty-row" style="display:none;">
            <!-- Slider number -->
            <div class="slide-col">
                <input type="text" class="slide-number" id="slide-num" disabled="disabled" value="">
                <!--<input type="hidden" name="slide['num'][]" value="<?php //echo $i; ?>">-->
            </div>
            <!-- Slide Background and images -->
            <div class="slide-col">
                <label><h4><?php _e( 'Slide Backgorund Color', 'dfr_simple_slider' ); ?></h4></label>
                <input name="slide[bg-color][]" type="text" value="" class="color-field" />
                <h4><?php _e( 'Image upload for desktop', 'dfr_simple_slider' ); ?></h4>
                <?php 
                //if ( $field['img'] ) :
            ...
            <div class="slide-col">
                <label><h4><?php _e( 'Content Align', 'dfr_simple_slider' ); ?></h4></label>
                <input type="radio" id="align-left" name="slide[align][]" value="left" ></input>
                <label for="align-left"><?php _e( 'Left', 'dfr_simple_slider' ) ?></label>
                <input type="radio" id="align-center" name="slide[align][]" value="center" ></input>
                <label for="align-center">
                    <?php _e( 'center', 'dfr_simple_slider' ); ?>
                    <!--<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                        width="80px" height="80px" viewBox="0 0 80 80" enable-background="new 0 0 80 80" xml:space="preserve">
                ...
                <input type="radio" id="align-right" name="slide[align]" value="right" ></input>
                <label for="align-right"><?php _e( 'Right', 'dfr_simple_slider' ) ?></label>
            </div>
            <!-- Slide title and content -->
            <div class="slide-col">
                ...
                <label><h4><?php _e( 'Slide Text', 'dfr_simple_slider' ) ?></h4></label>
                <textarea name="slide[text][]" cols="50" rows="10" maxlength="150" /></textarea>    
            </div>
            <!-- Slide button and link -->
            <div class="slide-col">
                <label><h4><?php _e( 'Slide Link', 'dfr_simple_slider' ) ?></h4></label>
                <input type="url" name="slide[link][]" value="" />
                <label><h4><?php _e( 'Slide button text', 'dfr_simple_slider' ); ?></h4></label>
                <input type="text" name="slide[btn-text][]" value="" />    
            </div>
            <div class="slide-col">
                <a class="button remove-row" href="#1"><?php _e( 'Remove Slide', 'dfr_simple_slider' ); ?></a>    
            </div>
...
function dfr_slider_save_post( $post_id ) {
    ...
    //$slide_number = $_POST['slide']['num'];
    $slide_bg_color = $_POST['slide']['bg-color'];
    $slide_image = $_POST['slide']['img'];
    $mobile_slide_image = $_POST['slide']['m-img'];
    $slide_align = $_POST['slide']['align'];
	$slide_tit = $_POST['slide']['title'];
	$slide_desc = $_POST['slide']['text'];
    $slide_link = $_POST['slide']['link'];
    $slide_btn_text = $_POST['slide']['btn-text'];
    ...
    if( is_array( $_POST['slide'] ) ):
        ...
        $count = count( $_POST['slide']['title'] );
        wp_die( var_export( $_POST['slide'] ) );
    else:
	...
	for ( $i = 0; $i < $count; $i++ ) {
		if ( $slide_tit[$i] != '' ) {
            ...
            //$new[$i]['slide']['num'] =  isset( $slide_number[$i] ) ? intval( $slide_number[$i] ) : null;
            $new[$i]['slide']['bg-color'] =  isset( $slide_bg_color[$i] ) ? sanitize_text_field( $slide_bg_color[$i] ) : '';
            $new[$i]['slide']['img'] =  isset( $slide_image[$i] ) ? intval( $slide_image[$i] ) : '';
            $new[$i]['slide']['m-img'] =  isset( $mobile_slide_image[$i] ) ? intval( $mobile_slide_image[$i] ) : '';
            $new[$i]['slide']['bg-color'] =  isset( $slide_bg_color[$i] ) ? sanitize_text_field( $slide_bg_color[$i] ) : '';
            $new[$i]['slide']['align'] =  isset( $slide_align[$i] ) ? sanitize_text_field( $slide_align[$i] ) : '';
			$new[$i]['slide']['title'] = isset( $slide_tit[$i] ) ? wp_strip_all_tags( $slide_tit[$i] ) : '';
			$new[$i]['slide']['text'] = isset( $slide_desc[$i] ) ? allowed_html_chars( $slide_desc[$i] ) : '';
            $new[$i]['slide']['link'] = isset( $slide_link[$i] ) ? valid_url( $slide_link[$i] ) : '';
            $new[$i]['slide']['btn-text'] = isset( $slide_btn_text[$i] ) ? wp_strip_all_tags( $slide_btn_text[$i] ) : '';
            /*
            $new['slide'][$i]['bg-color'] =  isset( $slide_bg_color[$i] ) ? sanitize_text_field( $slide_bg_color[$i] ) : '';
            $new['slide'][$i]['img'] =  isset( $slide_image[$i] ) ? intval( $slide_image[$i] ) : '';
            $new['slide'][$i]['m-img'] =  isset( $mobile_slide_image[$i] ) ? intval( $mobile_slide_image[$i] ) : '';
            $new['slide'][$i]['bg-color'] =  isset( $slide_bg_color[$i] ) ? sanitize_text_field( $slide_bg_color[$i] ) : '';
            $new['slide'][$i]['align'] =  isset( $slide_align[$i] ) ? sanitize_text_field( $slide_align[$i] ) : '';
			$new['slide'][$i]['title'] = isset( $slide_tit[$i] ) ? wp_strip_all_tags( $slide_tit[$i] ) : '';
			$new['slide'][$i]['text'] = isset( $slide_desc[$i] ) ? allowed_html_chars( $slide_desc[$i] ) : '';
            $new['slide'][$i]['link'] = isset( $slide_link[$i] ) ? valid_url( $slide_link[$i] ) : '';
            $new['slide'][$i]['btn-text'] = isset( $slide_btn_text[$i] ) ? wp_strip_all_tags( $slide_btn_text[$i] ) : '';
            */
		}
	}
    ...
    */
    ...
    //if ( isset( $_POST['slide'] ) ):
        ...
        foreach ( $_POST['slide'] as $i => $slide ) {
            ...
            $align_formats = array( 'left', 'center', 'right' );
            ...
            $new[] = array(
                ...
                'align' => isset( $slide['align'] ) && in_array( $slide['align'], $align_formats ) ? $slide['align'] : '',
    ...
    if ( ! empty( $clean ) ) { 
        ...
        delete_post_meta( $post_id, 'repeater_slider_content' ); 
    }
    */
}
